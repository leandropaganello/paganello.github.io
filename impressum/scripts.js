document.addEventListener('DOMContentLoaded', () => {
    ...
    if (logo) {
        logo.addEventListener('click', () => {
            ...
            if (clickCount === 5) {
                ...
                showSecretMessage("🌈 Rainbow Mode aktiviert!");
                document.body.classList.toggle('rainbow-theme');
            }
        });
        logo.addEventListener('contextmenu', (e) => {
            e.preventDefault();
            showSecretMessage('🔧 Rechtsklick erkannt!');
        });
    }
    ...
    // If on secret page, celebrate
    if (window.location.pathname.endsWith('secret.html')) {
        addConfetti();
        showSecretMessage('👑 Willkommen auf der geheimen Seite!');
    }
    ...
    if (contactForm) {
        contactForm.addEventListener('submit', (e) => {
            ...
            const subject = encodeURIComponent('Kontaktformular Nachricht von ' + name);
            const body = encodeURIComponent(message + '\n\nVon: ' + name + ' <' + email + '>');
            window.location.href = `mailto:leandro.paganello@credo-kirche.de?subject=${subject}&body=${body}`;
        });
    }
    ...
    // --- Secret Word Easter Egg ("star") ---
    const secretWord = 'star';
    let secretIndex = 0;
    document.addEventListener('keydown', (e) => {
        if (e.key.toLowerCase() === secretWord[secretIndex]) {
            secretIndex++;
            if (secretIndex === secretWord.length) {
                showSecretMessage('✨ Du hast den geheimen Stern gefunden!');
                secretIndex = 0;
                const footer = document.querySelector('footer p');
                if (footer && !document.getElementById('secret-link')) {
                    const a = document.createElement('a');
                    a.id = 'secret-link';
                    a.href = 'secret.html';
                    a.textContent = 'Geheimseite';
    ...
    // --- Footer double-click easter egg ---
    const pageFooter = document.querySelector('footer');
    if (pageFooter) {
        pageFooter.addEventListener('dblclick', () => {
            showSecretMessage('👑 Footer doppelt! Du bist der Boss.');
            addConfetti();
        });
    }

    // --- Additional key sequence: "rainbow" toggles rainbow theme ---
    const rainbowWord = 'rainbow';
    let rainbowIndex = 0;
    document.addEventListener('keydown', (e) => {
        if (e.key.toLowerCase() === rainbowWord[rainbowIndex]) {
            rainbowIndex++;
            if (rainbowIndex === rainbowWord.length) {
                document.body.classList.toggle('rainbow-theme');
                showSecretMessage('🌈 Farbmodus umgeschaltet!');
                rainbowIndex = 0;
            }
        } else {
            rainbowIndex = 0;
    ...
    // --- Bottom-of-page easter egg ---
    let bottomTriggered = false;
    window.addEventListener('scroll', () => {
        if (!bottomTriggered && (window.innerHeight + window.pageYOffset >= document.body.offsetHeight - 2)) {
            bottomTriggered = true;
            showSecretMessage('🎯 Du hast das Seitenende erreicht!');
            addConfetti();
        }
    });
